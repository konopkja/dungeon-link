  ...
  private createGroundEffectVisual(effect: GroundEffect): Phaser.GameObjects.Container {
    ...
    switch (effect.type) {
      ...
      case GroundEffectType.VoidGaze: {
        // Void Gaze: Cone telegraph showing boss gaze direction
        const coneAngle = effect.coneAngle ?? Math.PI / 3;
        const coneDirection = effect.coneDirection ?? 0;
        const radius = effect.radius;

        // Create cone - draw pointing RIGHT, use container rotation
      ...
      case GroundEffectType.EncroachingDarkness: {
        // Encroaching Darkness: Outer ring shrinking safe zone
        const innerRadius = effect.innerRadius ?? 0;

        // Outer danger ring (donut shape)
        const outerRing = this.add.graphics();
        outerRing.fillStyle(0x220033, 0.7);
        outerRing.beginPath();
        outerRing.arc(0, 0, effect.radius, 0, Math.PI * 2);
        outerRing.arc(0, 0, Math.max(1, effect.radius - innerRadius), 0, Math.PI * 2, true);
        outerRing.closePath();
        outerRing.fillPath();
        ...
        const safeZone = this.add.circle(0, 0, Math.max(1, effect.radius - innerRadius), 0x00ff00, 0.1);
    ...
    switch (effect.type) {
          ...
          if (quadrant && icon) {
            ...
            if (isActive) {
              ...
              if (!isSafe) {
                ...
                quadrant.setAlpha(0.3);
                icon.setAlpha(1);
              }
            } else {
              // Telegraph phase - show all quadrants
              quadrant.setAlpha(isSafe ? 0.15 : 0.25);
              icon.setAlpha(0.8);
            }
          }
        }
        // Don't apply default pulse to this effect
        return;
      }
      ...
      case GroundEffectType.VoidGaze: {
        ...
        if (cone) {
          ...
          if (isActive !== wasActive) {
            ...
            // Redraw cone with different intensity
            cone.clear();
            const coneAngle = effect.coneAngle ?? Math.PI / 3;
        ...
        if (warningText) {
          warningText.setVisible(!isActive);
        }
        // Don't apply default pulse
        return;
      }
      ...
      case GroundEffectType.EncroachingDarkness: {
        ...
        if (safeZone) {
          ...
          // Pulse warning as zone shrinks
          const urgency = innerRadius / effect.radius;
          const pulseRate = 200 - urgency * 150;
          const alpha = 0.1 + Math.sin(this.time.now / pulseRate) * 0.1 * urgency;
