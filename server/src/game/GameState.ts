        ...
        for (const enemy of currentRoom.enemies) {
          ...
          if (tauntingPet) {
            ...
            for (const player of state.players) {
              ...
              // Check if player is in the current room (with some padding for room transitions)
              const roomPadding = 50; // Allow targeting players slightly outside room bounds
              const inCurrentRoom =
